...
class ClockInClockOutScreenViewModel extends ChangeNotifier {
  ...
  ApiResponse<ClockInClockOutByEmployeeId> clockInClockOutByEmployeeId =
      ApiResponse.loading();
  ...
  Future<void> _setMediaUpload(ApiResponse<MediaUpload> response,
      int clockInOutReq, BuildContext context) async {
    if (response.data != null) {
  ...
  Future<void> getMediaUpload(String clockInImage, var data, int clockInOutReq,
  ...
  Future<void> submitClockInRequest(
      ClockInClockOutRequest request, BuildContext context) async {
    _setClockInResponse(ApiResponse.loading(), context);
    _myRepo
        .submitClockIn(request)
        .then(
            (value) => _setClockInResponse(ApiResponse.success(value), context))
        .onError((error, stackTrace) =>
            _setClockInResponse(ApiResponse.error(error.toString()), context));
  }

  void _setClockOutResponse(
      ApiResponse<PostApiResponse> response, BuildContext context) {
    if (response.data != null) {
      postApiResponse = response;
      Utils.toastMessage(postApiResponse.data!.mobMessage.toString());
  ...
  Future<void> submitRequestTimeOff(
      ClockInClockOutRequest request, BuildContext context) async {
    _setRequestTimeOffResponse(ApiResponse.loading(), context);
    _myRepo
        .submitRequestTimeOff(request)
        .then((value) =>
            _setRequestTimeOffResponse(ApiResponse.success(value), context))
        .onError((error, stackTrace) => _setRequestTimeOffResponse(
            ApiResponse.error(error.toString()), context));
  }

  void _setClockInClockOutByEmployeeId(
      ApiResponse<ClockInClockOutByEmployeeId> response) {
    if (response.data != null) {
  ...
  Future<void> fetchClockInClockOutByEmployeeId(
    ...
    _myRepo
        ...
        .then((value) =>
            _setClockInClockOutByEmployeeId(ApiResponse.success(value)))
        .onError((error, stackTrace) => _setClockInClockOutByEmployeeId(
            ApiResponse.error(error.toString())));
  }

  Future<ApiResponse<ClockInClockOutByEmployeeId>>
      fetchClockInClockOutByEmployeeId1(
          var clockInOutDate, var employeeId, var propertyId) async {
    ApiResponse<ClockInClockOutByEmployeeId> response = ApiResponse.loading();

    try {
      final value = await _myRepo.getClockInClockOutByEmployeeId(
          clockInOutDate, employeeId, propertyId);
