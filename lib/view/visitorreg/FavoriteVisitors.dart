...
class _FavoriteVisitorsScreenState extends State<FavoriteVisitorsScreen> {
  ...
  void fetchFavoriteVisitorList() async {
    viewModel
        .getFavoriteVisitors("ASC", "id", 1, 500, propertyId, userId)
        .then((response) {
                        ...
                        child: CheckboxListTile(
                          ...
                          onChanged: (bool? value) {
                            setState(() {
                              if (value == true) {
                                selectedItems.add(
                                    index); // Add the item's index to the selectedItems list
                              } else {
                                selectedItems.remove(
                                    index); // Remove the item's index from the selectedItems list
                              }
                            });
                          },
                        ),
                  ...
                  Navigator.pushReplacement(
                    ...
                    MaterialPageRoute(
                        builder: (context) => InvitationsScreen(
                            contact: null,
                            data: permissions,
                            favoritevisitor: selectedItemsData)),
                  );
                }
              : null,
