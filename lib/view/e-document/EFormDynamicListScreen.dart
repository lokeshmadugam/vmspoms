...
class EFormDynamicListScreen extends StatefulWidget {
  ...
  @override
  State<EFormDynamicListScreen> createState() => _EFormDynamicListScreenState();
}

class _EFormDynamicListScreenState extends State<EFormDynamicListScreen> {
  UserDetails userDetails = UserDetails();
  var viewmodel = EFormsViewModel();
  List<EFormCategoryItems> categoryItems = [];
  ...
  void _getUserDetails() async {
    ...
    fetchEFormCategoryNameList(userDetails.propertyId);
  ...
  Future<void> fetchEFormCategoryNameList(var propertyId) async {
    viewmodel.fetchEFormCategoryNameList(propertyId).then((response) {
      if (response.data?.status == 200) {
        if (response.data?.result != null) {
          var data = response.data!.result!.items;
          if (data != null) {
            ...
            child: Column(
              children: [
                MyDropDown(
                    ...
                    items: categoryItems
                        .map((item) => item.eformCategoryName)
                    ...
                    onchanged: (value) {
                      for (int i = 0; i < categoryItems.length; i++) {
                        if (value == categoryItems[i].eformCategoryName) {
                          categoryId = categoryItems[i].id!;
                          break;
                        }
                      }
                      fetchDynamicFormsList(categoryId, userDetails.propertyId);
                    }),
                SizedBox(
                ...
                if (fieldsList.isNotEmpty)
                  MyTextField(
                      ...
                      hintText: 'Comments',
                      preffixIcon: Icons.comment_bank_sharp,
                      maxLines: null,
                      labelText: 'Comments',
                      textInputType: TextInputType.multiline),
                if (fieldsList.isNotEmpty)
  ...
  void submitData() {
    ...
    viewmodel.submitEForm(data, context).then((value) {
